...
impl PullScheduler {
    ...
    pub fn tick(&mut self) -> AfsResult<PullSchedulerTick> {
        self.advance_by(Duration::ZERO)
    }

    pub fn advance_by(&mut self, elapsed: Duration) -> AfsResult<PullSchedulerTick> {
...
fn validate_interval(name: &'static str, interval: Duration) -> AfsResult<()> {
    if interval.is_zero() {
        return Err(afs_core::AfsError::InvalidState(format!(
