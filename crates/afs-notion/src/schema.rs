...
//! AgentFS validation issues that CLI and daemon paths can surface uniformly.
...
use afs_core::planner::PropertyValue;
    ...
    fn single_data_source(&self, file: &Path) -> Result<&DataSourceSchema, ValidationIssue> {
        match self.data_sources.as_slice() {
            ...
            _ => Err(issue(
                ...
                file,
                Some(1),
                "Notion database schema has multiple data sources; AFS cannot choose one for row writes yet",
...
fn validate_property(
    ...
    if !property.is_writable() {
        report.push(issue(
            ...
            "restore this property value or edit it in Notion until AFS supports this type",
        ));
        return;
    }
...
fn parse_shadow_frontmatter(shadow: &ShadowDocument) -> Result<ParsedCanonicalDocument, String> {
    parse_canonical_markdown(&afs_core::canonical::render_canonical_markdown(
...
mod tests {
    ...
    fn validates_only_changed_properties_for_existing_rows() {
        let shadow = ShadowDocument::from_synced_body(
            afs_core::model::RemoteId::new("row-1"),
            "",
            3,
            std::iter::empty::<afs_core::model::RemoteId>(),
        )
        .expect("shadow")
        .with_frontmatter("afs:\n  id: row-1\n  type: page\n  synced_at: now\n  remote_edited_at: now\ntitle: Task\nFormula: computed\nStatus: Todo\n");
        let parsed = parse_canonical_markdown(
            "---\nafs:\n  id: row-1\n  type: page\n  synced_at: now\n  remote_edited_at: now\ntitle: Task\nFormula: computed\nStatus: Blocked\n---\n# Body\n",
        )
        .expect("parse row");
