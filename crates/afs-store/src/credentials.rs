...
const PRIMARY_KEYCHAIN_SERVICE: &str = "afs";

#[cfg(any(test, target_os = "macos"))]
const COMPAT_KEYCHAIN_SERVICES: [&str; 2] = [PRIMARY_KEYCHAIN_SERVICE, "loc"];
...
mod tests {
    ...
    #[test]
    fn keychain_services_include_loc_compatibility_alias() {
        assert_eq!(COMPAT_KEYCHAIN_SERVICES, [PRIMARY_KEYCHAIN_SERVICE, "loc"]);
    }

    #[test]
    fn windows_target_names_include_locality_compatibility_alias() {
        assert_eq!(
            windows_target_names("workspace"),
            [
                primary_windows_target_name("workspace"),
                "ai.codeflash.locality:workspace".to_string(),
            ]
        );
    }
}
