...
| `child_page` | Markdown link and structural enumeration | No direct block write | fixture, live read, live move-blocked | Parent pages show a readable link to the child page's stable Notion URL; edit, move, rename, or delete the child through its projected Markdown file/directory. Editing or moving the rendered parent-page link is blocked before journaled apply because copying or patching that link would create the wrong child-page semantics. New child pages are created through page/entity creation, not block edits. |
...
| `link_to_page` | Markdown link to Notion URL | Read/delete/move only | fixture, live read/move, blocked-write regression | Page/database target ID is preserved in the link target. Moving the rendered link appends a copy of the existing `link_to_page` payload and archives the old block so it does not degrade into a paragraph link. Direct retargeting is blocked because Notion ignores direct target PATCHes and replacement needs undo-aware block identity support. |
