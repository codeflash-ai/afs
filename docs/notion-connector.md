...
- supported rich-text spans: bold, italic, strikethrough, underline, code, external links, inline equations, Notion page links, database links whose target ID matches a rendered database mention, explicit `@date(...)` date mentions, legacy `afs://` page links, and unchanged preimage mentions such as dates;
