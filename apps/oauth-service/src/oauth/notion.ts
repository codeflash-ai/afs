...
export function notionAuthorizeUrl(env: BrokerEnv, redirectUri: string, state: string): string {
  ...
  url.searchParams.set("client_id", requireEnv(env.AFS_NOTION_CLIENT_ID, "AFS_NOTION_CLIENT_ID"));
...
async function notionTokenRequest(
  ...
  const clientId = requireEnv(env.AFS_NOTION_CLIENT_ID, "AFS_NOTION_CLIENT_ID");
  const clientSecret = requireEnv(env.AFS_NOTION_CLIENT_SECRET, "AFS_NOTION_CLIENT_SECRET");
  const response = await fetcher(`${notionApiBaseUrl(env)}/v1/oauth/token`, {
    method: "POST",
    headers: {
      Authorization: `Basic ${btoa(`${clientId}:${clientSecret}`)}`,
      "Content-Type": "application/json",
      "Notion-Version": env.AFS_NOTION_VERSION ?? "2022-06-28"
    },
...
function notionAuthBaseUrl(env: BrokerEnv): string {
  return (env.AFS_NOTION_AUTH_BASE_URL ?? "https://api.notion.com").replace(/\/+$/, "");
}

function notionApiBaseUrl(env: BrokerEnv): string {
  return (env.AFS_NOTION_API_BASE_URL ?? "https://api.notion.com").replace(/\/+$/, "");
}
